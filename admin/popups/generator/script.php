<?php if ( ! defined( 'ABSPATH' ) ) exit;
	...
	*/
	...
	$id = $val->id;	
	// Style 
	$popupOverlay = !empty( $param['overlay'] ) ? 'true' : 'false';
	$location = !empty( $param['location'] ) ? $param['location'] : 'bottomRight';
	// Popup Settings
	$triggers = !empty( $param['triggers'] ) ? $param['triggers'] : 'click';
	...
	$set_cookie = !empty( $param['set_cookie'] ) ? 'true' : 'false';
	...
	$close_overlay = !empty( $param['close_overlay'] ) ? 'true' : 'false';
	$close_esc = !empty( $param['close_esc'] ) ? 'true' : 'false';
	
	// Device Screen
	$max_screen_enable = !empty($param['max_screen_enable']) ? 'true' : 'false';
	$max_screen = !empty($param['max_screen']) ? $param['max_screen'] : '1024';
	$min_screen_enable = !empty($param['min_screen_enable']) ? 'true' : 'false';
...
jQuery(document).ready(function() {
	jQuery('#lg-popup-<?php echo $id;?>').lgPopup({
		...
		openPopup: 'open-popup-<?php echo $id;?>',
		closePopup: 'close-popup-<?php echo $id;?>',				
		setCookie: <?php echo $set_cookie;?>,
		cookieDays: <?php echo $auto_close_time;?>,
		cookieName: 'lg-popup-<?php echo $id;?>',
		popupAnalytic: true,
		popupID: '<?php echo $id;?>',		
	})
});